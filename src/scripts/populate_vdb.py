"""Pobla una base de datos de Vertex AI Vector Search con contenido de dos APIs.

Este script realiza las siguientes acciones:
1.  Se autentica contra la API para obtener un token de sesión.
2.  Consume la API de posts (`/api/v1/posts/meds`) usando el token.
...
from turbo_firecrawl.firecrawl_scraper import EnhancedFirecrawlClient
...
def fetch_data(url: str, search_term: str, auth_token: str) -> List[Dict[str, Any]]:
    ...
    while current_url:
        try:
            ...
            # El `params` solo se usa en la primera petición
            params = None
...
def prepare_posts_for_upsert(posts: List[Dict[str, Any]], firecrawl_client: Optional[EnhancedFirecrawlClient], search_term: str) -> List[Dict[str, Any]]:
    ...
    # Inicializar cliente de GCS para guardar contenido en el mismo bucket
    storage_client = storage.Client(project=config.project_id)
    content_bucket_name = config.gcs_bucket_name  # Usar el mismo bucket que los embeddings
    ...
    for post in posts:
        try:
            ...
            # Guardar el contenido completo en GCS
            content_data = {
                'id': doc_id,
            ...
            # Para Vector Search, usar solo un resumen del contenido
            summary_content = f"Título: {name}. Descripción: {description[:200]}..."
            
            document_data = {
                'id': doc_id,
                'content': summary_content,  # Contenido resumido para embeddings
...
def main():
    ...
    # --- VERIFICACIÓN INTELIGENTE DEL MÉTODO DE ACTUALIZACIÓN ---
    ...
    try:
        ...
        time.sleep(5) # Dar un respiro antes de verificar
